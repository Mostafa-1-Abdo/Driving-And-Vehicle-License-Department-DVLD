    ...
    namespace DVLD.Logic
    {
        public class clPerson
        {
            ...
            public clCountry Country => CountryID != -1 ? clCountry.Find(CountryID) : null;

            public string FullName => !string.IsNullOrWhiteSpace(ThirdName) ?
                $"{FirstName} {SecondName} {ThirdName} {LastName}" :
                $"{FirstName} {SecondName} {LastName}";

            private clPersonDTO PersonDTO => new clPersonDTO(ID, (byte)Gender, FirstName, SecondName, ThirdName, LastName,
